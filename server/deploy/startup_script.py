...
def generate_startup_script(
    ...
    deploy_package_base64: str | None = None,
    ...
    Args:
        ...
        import_source: Git URL or tarball path for cg import
        branch: Optional git branch/tag for import
        deploy_package_base64: Optional base64-encoded tarball to embed
    ...
    if deploy_package_base64:
        # Embedded package - extract and use local path
        package_extraction = f'''
# Extract embedded deploy package
PACKAGE_DIR="/tmp/comfygit_deploy_{deployment_id}"
mkdir -p "$PACKAGE_DIR"
echo '{deploy_package_base64}' | base64 -d > "$PACKAGE_DIR/deploy_package.tar.gz"
tar -xzf "$PACKAGE_DIR/deploy_package.tar.gz" -C "$PACKAGE_DIR"
IMPORT_SOURCE="$PACKAGE_DIR"
'''
    else:
        package_extraction = f'''
